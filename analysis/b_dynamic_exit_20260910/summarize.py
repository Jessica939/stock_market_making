"""Readable report and figure from the predeclared scenario grid."""
import json
...
def main():
    ...
    previous=m['runs'][0]['run']
    policies=['baseline','guard','guard_1s','guard_3s','baseline_1s','baseline_3s']
    names=dict(baseline='原规则',guard='减仓价格保护',guard_1s='价格保护 + 1 秒超时',
               guard_3s='价格保护 + 3 秒超时',baseline_1s='原规则 + 1 秒超时',baseline_3s='原规则 + 3 秒超时')
    def maincase(run,policy):
        return next(r for r in rows if r['run']==run and r['policy']==policy and
                    r['queue']=='displayed' and r['latency']==.05 and r['depth_removal']==200)
    keys=('run','queue','latency','depth_removal')
    def comparator(row,policy):
        return next(r for r in rows if r['policy']==policy and all(r[k]==row[k] for k in keys))
    guards=[r for r in rows if r['policy']=='guard']
    improvements=[r['terminal_equity']-comparator(r,'baseline')['terminal_equity'] for r in guards]
    timeout1=[r['terminal_equity']-comparator(r,'guard')['terminal_equity'] for r in rows if r['policy']=='guard_1s']
    timeout3=[r['terminal_equity']-comparator(r,'guard')['terminal_equity'] for r in rows if r['policy']=='guard_3s']
    summary=dict(scenarios=len(rows),negative_scenarios=sum(r['terminal_equity']<0 for r in rows),
        guard_better_than_base=sum(x>0 for x in improvements),guard_comparisons=len(improvements),
        guard_1s_worse_than_guard=sum(x<0 for x in timeout1),guard_3s_worse_than_guard=sum(x<0 for x in timeout3),
        max_abs_inventory=max(r['peak_abs_position'] for r in rows),current={})
    table=[]
    for policy in policies:
        r=maincase(current,policy)
        trace=[json.loads(l) for l in (OUT/(current+'_'+policy+'_episodes.jsonl')).read_text().splitlines()]
        # Independent final ledger/episode audit on detailed main-scenario traces.
        fills=[json.loads(l) for l in (OUT/(current+'_'+policy+'_fills.jsonl')).read_text().splitlines()]
        position=0;cash=0.;peak=0
        for f in fills:
            assert f['before']==position
            position+=(1 if f['side']=='bid' else -1)*f['volume']
            cash+=(1 if f['side']=='ask' else -1)*f['volume']*f['price']
            peak=max(peak,abs(position))
            assert f['after']==position
        assert position==r['final_position'] and abs(cash-r['cash'])<1e-7 and peak==r['peak_abs_position']
        if position==0:
            assert abs(sum(e['pnl'] for e in trace)-cash)<1e-7
        summary['current'][policy]=dict(episode_median_seconds=statistics.median(e['seconds'] for e in trace),
            losing_episodes=sum(e['pnl']<0 for e in trace),worst_episode=min(e['pnl'] for e in trace))
        table.append(f"| {names[policy]} | {r['terminal_equity']:.1f} | {r['stats'].get('volume',0)} | {r['peak_abs_position']} | {r['average_abs_inventory']:.3f} | {r['observed_mid_drawdown']:.1f} | {r['stats'].get('timeout_volume',0)} |")
    ranges=[]
    for run,label in [(previous,'16:57—17:11'),(current,'17:11—17:18')]:
        for policy in policies[:4]:
            z=[r for r in rows if r['run']==run and r['policy']==policy]
            diffs=[r['terminal_equity']-comparator(r,'baseline')['terminal_equity'] for r in z]
            ranges.append(f"| {label} | {names[policy]} | {min(r['terminal_equity'] for r in z):.1f} 至 {max(r['terminal_equity'] for r in z):.1f} | {min(diffs):+.1f} 至 {max(diffs):+.1f} |")
    fig,axes=plt.subplots(1,2,figsize=(11,4.8),sharey=False)
    labels=['Base','Exit guard','Guard + 1s','Guard + 3s']
    for ax,run,title in zip(axes,[previous,current],['Earlier window (~14.7 min)','Current window (~6.1 min)']):
        for i,policy in enumerate(policies[:4]):
            z=[r['terminal_equity'] for r in rows if r['run']==run and r['policy']==policy]
            ax.plot([i,i],[min(z),max(z)],color='#9baec8',lw=6,solid_capstyle='round')
            ax.scatter(i,maincase(run,policy)['terminal_equity'],color='#185a93',s=65,zorder=3)
        ax.axhline(0,color='#b74b4b',lw=1,ls='--')
        ax.set_xticks(range(4),labels,rotation=15)
        ax.set_title(title)
        ax.set_ylabel('Terminal equity change (before fees)')
        ax.grid(axis='y',alpha=.2)
        ax.spines[['top','right']].set_visible(False)
    fig.suptitle('Exit protection reduces modeled losses; no tested scenario is profitable',fontsize=13)
    fig.text(.5,.015,'Line: range across 8 execution assumptions (not a confidence interval). Dot: predeclared main case.',ha='center',fontsize=9)
    fig.tight_layout(rect=(0,.04,1,.94))
    fig.savefig(OUT/'scenario_comparison.png',dpi=160)
    plt.close(fig)
    report=f'''# B 减仓价格保护与超时：动态仓位回放

本轮结论：价格保护是有一致改善方向的候选，但仍不足以使 B 盈利。两段行情、八组成交假设下，价格保护相对原规则在 **{summary['guard_better_than_base']} / {summary['guard_comparisons']}** 个对应比较中都少亏。六种规则总计 **{summary['scenarios']} 个组合全部亏损**；不能把减少亏损解释为已经找到盈利策略。
...
- 仓位加全部未成交买单、仓位减全部未成交卖单分别检查 ±100，挂单总量检查 200。所有 {summary['scenarios']} 个情景最大绝对持仓不超过 {summary['max_abs_inventory']} 股；这不是其他行情下的风险上界。

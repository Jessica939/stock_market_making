"""Load the isolated baseline-refine-hybird strategy; connect only with --live."""
import argparse
...
def load_quote_definitions():
    ...
    fills = importlib.import_module(PACKAGE + '.fill_stream')
    namespace = dict(math=math, CycleSettings=cycle.CycleSettings,
                     MarketMakingSettings=hybrid.MarketMakingSettings,
                     HybridPosition=hybrid.HybridPosition, FillStream=fills.FillStream,
    ...
    for cell in notebook['cells']:
        ...
        if cell['cell_type'] == 'code' and 'strategy' in cell.get('metadata', {}).get('tags', []):
            compile(source, str(DIRECTORY/'strategy_with_logging.ipynb'), 'exec')
    ...
    hybrid.HybridPosition(market_making=namespace['B_MM_SETTINGS'],
                          **namespace['B_POSITION_SETTINGS'])
...
def load_strategy():
    ...
    namespace = {'__name__': 'baseline_refine_hybird', '__package__': PACKAGE, '__file__': str(path)}
    for cell in notebook['cells']:
        if cell['cell_type'] == 'code' and 'strategy' in cell.get('metadata', {}).get('tags', []):
...
def main():
    ...
    if args.check:
        ...
        print(json.dumps(dict(strategy=strategy['STRATEGY_VERSION'],
                              ...
                              b_market_making=asdict(strategy['B_MM_SETTINGS']),
                              ordering_strategy='passive MM limits; competitive cycle entries; passive then marketable limit exits',
                              ioc_enabled=False,
                              fill_confirmation='shared poll_new_trades, deduplicated by instrument/trade_id',

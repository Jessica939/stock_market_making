...
3-tick 门槛、2 股基础仓位、5 秒持仓和 180 秒周期来自历史与实盘观察。高 edge 最多放大到 50 股是虚拟盘上的激进配置，并没有被原 2 股实盘样本直接验证。放大仓位会改变可成交 VWAP，因此程序会按整笔 VWAP 重新验收并在必要时降档；这仍不是盈利保证。历史盘口包含原策略行为，无法重建新策略参与后的反事实队列和成交。

...
DEFAULTS = dict(symbols=['PHILIPS_A', 'PHILIPS_B'], session_seconds=1800,
                closeout_seconds=120, loop_seconds=0.25, max_updates_per_second=20,
                position_limit=100, max_outstanding_volume=200, max_order_lots=20,
                max_net_lots=30, max_book_age_seconds=2.0, max_pair_time_gap_seconds=0.75,
                max_spread_ticks=20, boundary_volume=20000, boundary_volume_tolerance=0.25,
                boundary_min_distance_ticks=5, max_depth_ticks=4, max_depth_levels=5,
                feature_volume_cap=100, entry_slippage_ticks=1, exit_slippage_ticks=3,
                settlement_seconds=0.25, fee_per_lot=0.0, max_session_loss=50.0,
                max_drawdown=60.0, shutdown_grace_seconds=5.0,
                initial_cleanup_seconds=10.0,
                max_mid_jump_ticks=20, jump_cooldown_seconds=5.0)
...
def validate_config(config):
    ...
    for key in ('session_seconds', 'closeout_seconds', 'loop_seconds', 'max_session_loss',
                'max_drawdown', 'max_book_age_seconds', 'settlement_seconds',
                'shutdown_grace_seconds', 'initial_cleanup_seconds', 'max_pair_time_gap_seconds',
                'max_mid_jump_ticks', 'jump_cooldown_seconds'):
    ...
    for key, maximum in (('position_limit', 100), ('max_outstanding_volume', 200),
                         ('max_updates_per_second', 22), ('max_order_lots', 100), ('max_net_lots', 100)):
        if isinstance(config[key], bool) or not isinstance(config[key], int) or not 1 <= config[key] <= maximum:
            raise ValueError(f'{key} must be integer 1..{maximum}')
    for key in ('fee_per_lot', 'entry_slippage_ticks', 'exit_slippage_ticks'):
        if (isinstance(config[key], bool) or not isinstance(config[key], (int, float))
                or config[key] < 0 or not math.isfinite(config[key])):
            raise ValueError(f'{key} must be finite and nonnegative')
    required_slippage = (config['entry_slippage_ticks'] + config['exit_slippage_ticks']) / 2
    for key in ('slippage_ticks', 'ml_slippage_ticks_per_side'):
        config.setdefault(key, required_slippage)
        value = config[key]
        if (isinstance(value, bool) or not isinstance(value, (int, float))
                or not math.isfinite(value) or value < required_slippage):
            raise ValueError(f'{key} must cover entry+exit slippage: at least {required_slippage:g} ticks per side')
    validate_market_config(config)


class Journal:
    def __init__(self, directory=None, *, strategy='cleanup', mode='live', config=None, **metadata):
        self.storage = RunStorage(strategy, directory=directory, mode=mode, config=config, **metadata)
    ...
    def emit(self, kind, **fields):
        if self.failed or self.closed:
            return
        try:
            self.file.write(json.dumps(dict(type=kind, recorded_at=datetime.now(timezone.utc).isoformat(),
                                             **fields), ensure_ascii=False, allow_nan=False, default=str) + '\n')
        except (OSError, ValueError, TypeError) as exc:
            self.failed = True
            print(f'Journal failed; entries disabled, cleanup continues: {exc}', file=sys.stderr)
    def close(self):
        if not self.closed:
            try:
                self.file.close()
...
class Feed:
    ...
    """
    def __init__(self, exchange, symbols, config, clock=time.monotonic, epoch=time.time,
                 journal=None):
    ...
    def one(self, iid, reducing=False, reducing_side=None):
        ...
        for side in ('bids', 'asks'):
            levels = raw.get(side, []) if isinstance(raw, dict) else getattr(raw, side, [])
            items = []
            for level in levels or ():
                items.append(list(level) if isinstance(level, (tuple, list)) else
                             [getattr(level, 'price', None), getattr(level, 'volume', None)])
            sides[side] = items[:5]
            sides[side + '_outermost'] = items[-1] if items else None
        self._emit('raw_book', instrument=iid, reducing=bool(reducing or reducing_side),
                   timestamp=raw.get('timestamp') if isinstance(raw, dict) else getattr(raw, 'timestamp', None),
                   **sides)
        book = clean_book(raw, self.instruments[iid].tick_size, self.epoch(), self.config,
                          for_execution=bool(reducing or reducing_side),
                          reference_price=self._reference(iid), boundary_tracker=self.trackers[iid])
        signature = json.dumps(self._json_safe(book.get('boundary_levels', [])), sort_keys=True, default=str)
        if signature != self.boundary_signatures.get(iid):
            self.boundary_signatures[iid] = signature
            self._emit('boundary_change', instrument=iid, levels=book.get('boundary_levels', []),
    ...
    def frame(self):
        ...
        for iid in self.symbols:
            ...
            for trade in ticks:
                ...
                row = dict(price=price, volume=getattr(trade, 'volume', None),
                           side=getattr(trade, 'aggressor_side', None),
                           timestamp=getattr(trade, 'timestamp', None), trade_id=getattr(trade, 'trade_id', None))
                ...
                if (not isinstance(price, bool) and isinstance(price, (int, float))
                        ...
                        and -1 <= self.epoch() - stamp <= min(2.0, self.config['max_book_age_seconds'])
                        and stamp >= self.last_trade_timestamps.get(iid, -math.inf)):
                    self.last_prices[iid], self.last_trade_timestamps[iid] = price, stamp
        books = {}
        for iid in self.symbols:
            try:
                book = self.one(iid)
        ...
        for iid, book in books.items():
            ...
            if old is not None and abs(book['mid'] - old) / book['tick'] > self.config['max_mid_jump_ticks']:
                self.cooldown_until = self.clock() + self.config['jump_cooldown_seconds']
    ...
    def holding_frame(self):
        ...
        for iid in self.symbols:
            ...
            if reasons or book['bid'] is None or book['ask'] is None:
                raise UnusableBook('holding prices unavailable: ' + ', '.join(sorted(reasons)))
            books[iid] = dict(book, mid=(book['bid']+book['ask'])/2, signal_usable=True)
        if max(b['timestamp'] for b in books.values()) - min(b['timestamp'] for b in books.values()) > self.config['max_pair_time_gap_seconds']:
    ...
    def liquidation_equity(self, *, require_bounded=True):
        ...
        for iid in self.symbols:
            ...
            if (isinstance(q, bool) or not isinstance(q, int)
                    or isinstance(cash, bool) or not isinstance(cash, (int, float))
            ...
            if q:
                ...
                _, available = ioc_plan(book, side, abs(q), self.config['exit_slippage_ticks'])
                if require_bounded and available < abs(q):
                    raise UnusableBook('insufficient bounded depth to value liquidation')
...
class Session:
    def __init__(self, policy, kind, exchange, feed, config, journal, clock, sleep,
                 started_at=None, terminal_quantity=None):
        self.policy, self.kind, self.feed, self.config, self.journal = policy, kind, feed, config, journal
        self.clock, self.sleep = clock, sleep
        self.start = clock() if started_at is None else started_at
        self.executor = Executor(exchange, config['symbols'], feed, config, journal, clock, sleep,
                                 terminal_quantity=terminal_quantity)
        self.executor.entry_deadline = self.start + config['session_seconds'] - config['closeout_seconds']
        ...
        try:
            ...
            if not self.initialized:
                if any(positions.values()):
                    ...
                    if elapsed >= min(self.config['initial_cleanup_seconds'], self.config['session_seconds']):
                        self.risk_halt = True
                        self.fatal_error = 'inherited positions remain at startup cleanup deadline'
            ...
            stopping = (self.risk_halt or self.executor.halted or self.journal.failed
                        or elapsed >= self.config['session_seconds'] - self.config['closeout_seconds'])
            if stopping:
                self.pending = None
            ...
            if self.clock() >= self.executor.entry_deadline:
                ...
                return
            self.journal.emit('decision', elapsed=elapsed, reason=decision['reason'],
                              targets=decision['targets'], diagnostics=decision.get('diagnostics', {}),
                              liquidation_equity=equity, pnl_change=equity - self.baseline,
                              books=frame['books'])
            if delayed:
            ...
            try:
                ...
                self.policy.decide({'now': self.clock(), 'books': {}, 'trades': {}},
                                   actual, elapsed)
            except Exception as invalidation_error:
                self.risk_halt = True
                self.fatal_error = str(invalidation_error)
                self.journal.emit('policy_invalidation_error', error=str(invalidation_error))
            ...
            self.journal.emit('execution_halt', error=str(exc), hard_fault=self._hard_fault())
            if self._hard_fault():
    ...
    def finish(self, grace=5.0):
        ...
        try:
            while not self._hard_fault() and self.clock() < deadline:
                ...
                if (positions is not None and not any(positions.values())
                        and not self._hard_fault() and getattr(self.executor, '_pending', None) is None):
                    break
        ...
        try:
            ...
            if (not isinstance(all_positions, Mapping)
                    or any(i not in all_positions for i in self.config['symbols'])
                    or any(isinstance(q, bool) or not isinstance(q, int) for q in all_positions.values())):
                raise ExecutionFault('final account positions malformed')
            positions = {i: all_positions[i] for i in self.config['symbols']}
            final_snapshot_agrees = reconciled_positions is not None and positions == reconciled_positions
            if reconciled_positions is not None and not final_snapshot_agrees:
                self.journal.emit('final_position_mismatch', reconciled=reconciled_positions,
                                  observed=positions)
            unmanaged = {i: q for i, q in all_positions.items() if i not in self.config['symbols'] and q}
        except Exception as exc:
            unmanaged = None
        ...
        confirmed_flat = (final_snapshot_agrees and positions is not None and not any(positions.values())
                          and not self._hard_fault() and getattr(self.executor, '_pending', None) is None
                          and getattr(self.executor, 'account_consistent', True) and unmanaged == {})
        summary = dict(final_positions=positions, unmanaged_positions=unmanaged, flat=confirmed_flat,
                       ...
                       hard_fault=self._hard_fault(), fault_reason=getattr(self.executor, 'fault_reason', None),
                       liquidation_equity=self.last_equity,
                       pnl_change=(self.last_equity - self.baseline
                                   if self.last_equity is not None and self.baseline is not None else None),
                       pnl_scope='selected instruments, baseline after inherited-position cleanup',
                       cycles=self.cycles, execution_halted=self.executor.halted,
...
def main(policy_class, kind, folder, argv=None, *, session_class=Session,
    ...
    parser.add_argument('--duration', type=float, help='Total session seconds including warmup and closeout')
    parser.add_argument('--replay', help='Full-depth JSONL(.gz) path/glob, one chronological recording session')
    parser.add_argument('--replay-tick-size', type=float, default=0.1)
    parser.add_argument('--fill-fraction', type=float, default=0.5, help='Offline accessible share of displayed volume')
    parser.add_argument('--seed', type=int, default=7)
    parser.add_argument('--price-data-dir', type=Path, default=MARKET_DIR)
    parser.add_argument('--log-dir', '--output-dir', dest='output_dir', type=Path,
                        default=RUNS_DIR / kind, help='Parent directory for new run folders')
    args = parser.parse_args(argv)
    config = dict(DEFAULTS)
    config_path = args.config if args.config is not None else Path(folder) / 'config.json'
    ...
    journal = Journal(args.output_dir, strategy=kind, mode=args.mode, config=config,
                      replay_source=args.replay, replay_tick_size=args.replay_tick_size,
                      fill_fraction=args.fill_fraction, seed=args.seed)
    journal.emit('settings', mode=args.mode, strategy_kind=kind, config=config,
    ...
    try:
        if args.mode == 'live':
            ...
            exchange = RecordingExchange(Exchange(max_nr_trade_history=10000), args.price_data_dir)
            exchange.connect()
            ...
            if any(q for i, q in all_positions.items() if i not in config['symbols']):
                raise ValueError('unmanaged positions outside the selected pair; no new trades started')
            while (exchange.is_connected() and not session.stop_requested
            ...
            print('Offline functional simulation: results are not a live profitability estimate.', flush=True)
            frames = (demo_frames(config['symbols'], config['session_seconds'], seed=args.seed)
                      if args.mode == 'demo' else read_frames(args.replay, config['symbols'], args.replay_tick_size))
            clock = SimClock()
            exchange = ReplayExchange(config['symbols'], clock, config['fee_per_lot'], args.fill_fraction)
            first_epoch = None
            for raw in frames:
        ...
        try:
            if session is not None and exchange.is_connected():
                ...
                remaining = max(0.0, session.executor.reduction_deadline - session.clock())
                summary = session.finish(min(config['shutdown_grace_seconds'], remaining))
            elif session is not None:
                journal.emit('session_end', flat=False, final_positions=None,

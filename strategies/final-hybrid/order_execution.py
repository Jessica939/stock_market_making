...
class LimitedExchange:
    ...
    def __init__(self, exchange, *, max_outstanding_volume=200,
        ...
        if self.position_limit > 100:
            raise ValueError('per-instrument position_limit cannot exceed 100')
    ...
    def amend_order(self, instrument_id, *, order_id, volume):
        ...
        self._check_worst_position(
            instrument_id, side_name(orders[order_id].side), orders, volume,
...
class QuoteManager:
    ...
    def _desired(self, quote):
        ...
        if quote.get('order_type', 'limit') != 'limit':
            raise ValueError('QuoteManager requires limit orders with observable resting volume')
        desired = {}
        if 'replenish_side' in quote:
            if quote['replenish_side'] not in ('bid', 'ask'):
                raise ValueError('replenish_side must be bid or ask')
            desired['replenish_side'] = quote['replenish_side']
        ...
        bounds = ('min_position', 'max_position')
        if any(key in quote for key in bounds):
            if 'target_position' in quote or not all(key in quote for key in bounds):
                raise ValueError('position band requires both bounds and no target_position')
            for key in bounds:
                value = quote[key]
                if type(value) is not int or abs(value) > self.position_limit:
                    raise ValueError(key + ' must be an integer within position limits')
                desired[key] = value
            if desired['min_position'] > desired['max_position']:
    ...
    def _orders(self, instrument_id):
        ...
        for order_id, order in raw_orders.items():
            ...
            if side_name(order.side) not in ('bid', 'ask'):
                raise ValueError('outstanding order side must be bid or ask')
            orders[order_id] = (side_name(order.side), self._price(order.price), volume)
        return orders
    ...
    def _capacity(self, side, desired, position, instrument_id=None):
        ...
        if 'min_position' in desired:
            room = min(room, desired['max_position'] - position if side == 'bid'
                       else position - desired['min_position'])
    ...
    def reconcile(self, instrument_id, quote):
        ...
        for side in ('bid', 'ask'):
            ...
            for order_id in reversed(ids):
                try:
                    ...
                    expand_cycle = (bool(desired.get('target_position'))
                                    or desired.get('replenish_side') == side) and total < capacity
        ...
        for side in ('bid', 'ask'):
            ...
            if self.fill_stream is not None:
                self.fill_stream.refresh(instrument_id)
                confirmed = self.fill_stream.confirmed_volume(instrument_id, response.order_id)
                if confirmed > volume:
                    raise RuntimeError('confirmed fills exceed the submitted order volume')
                result.setdefault('execution_reports', []).append(dict(
                    order_id=response.order_id, order_type='limit', submitted_volume=volume,
                    confirmed_fill_volume=confirmed, not_yet_confirmed_volume=volume-confirmed))
